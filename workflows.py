...
QWEN_IMAGE_2512_GEN = {
  ...
  "91": {
    "inputs": {
      "value": "Urban alleyway at dusk. Female person walking the street. Full body portrait.\n"
    },
...
def get_workflow(name, mode="gen", img_count=0):
    if name == "flux-kontext-dev":
        ...
        # Multi-image logic for Qwen edit (1–3 images)
        if mode == "edit":
            if img_count == 1:
                return copy.deepcopy(QWEN_IMAGE_EDIT)
            elif img_count == 2:
                return copy.deepcopy(QWEN_IMAGE_EDIT_2IMG)
            elif img_count == 3:
                return copy.deepcopy(QWEN_IMAGE_EDIT_3IMG)
        # Fallback to single-image workflow
        return copy.deepcopy(QWEN_IMAGE_EDIT)
    elif name == "qwen-image":
        # Text-to-image workflow
        return copy.deepcopy(QWEN_IMAGE_2512_GEN)
